...
public class CatalogService<TEntity, TInput, TDto>(AppDbContext db, ICatalogDefinition<TEntity, TInput, TDto> definition) : ICatalogService<TInput, TDto> where TEntity : Entity, new()
{
    ...
    public async Task DeleteAsync(Guid id, CancellationToken ct)
    {
        ...
        if(entity is Client && await db.BankAccounts.AnyAsync(a=>a.ClientId==id,ct))
            throw new AppException(409,"Ce client possède des comptes bancaires. Utilisez la confirmation détaillée pour supprimer le client et ses éléments liés.");
