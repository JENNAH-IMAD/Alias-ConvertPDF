...
export default function ClientsPage(){
 const {user,can}=useAuth();const [page,setPage]=useState(1);const {data,error,loading,reload}=useApi<Page<Client>>('/clients?page='+page+'&pageSize=20');const clients=data?.items||[];
 ...
 <section className="ui-hero flex flex-wrap items-center justify-between gap-5"><div><p className="ui-eyebrow">RÉFÉRENTIEL</p><h1 className="ui-title">Clients</h1><p className="ui-description">Retrouvez leurs coordonnées, leurs informations fiscales et leurs banques.</p></div>{can('clients.write')&&<Button variant="ghost" className="button-primary" onClick={()=>setEditing(null)}><Plus size={17}/> Nouveau client</Button>}</section>
 <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2 font-semibold"><Users size={18} className="accent-text"/> Tous les clients <Badge variant="outline" className="ui-tag">{data?.total??'—'}</Badge></div><span className="muted text-xs">Répertoire clients</span></div>
 <LoadState error={error} loading={loading} empty={!clients.length}/>
 <section className="grid gap-5 md:grid-cols-2 2xl:grid-cols-3">{clients.map(client=><Card asChild key={client.id}><article key={client.id} className="ui-card client-card"><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-center gap-3"><Button variant="ghost" type="button" className="client-photo-button" title="Modifier la photo ou le logo" aria-label={`Modifier la photo de ${client.name}`} disabled={!can('clients.write')} onClick={()=>setPhotoClient(client)}><ClientAvatar client={client}/><span className="photo-camera"><Camera size={11}/></span></Button><div className="min-w-0"><h2 className="truncate text-base font-bold" title={client.name}>{client.name}</h2><p className="muted mt-1 truncate text-xs" title={client.legalName}>{client.legalName}</p></div></div><span className={client.status==='Actif'?'status-positive':'ui-tag'}>{client.status}</span></div>
 ...
 <div className="mt-5 flex flex-wrap items-center gap-2"><Landmark size={14} className="muted"/>{client.banks.length?client.banks.map(bank=><Badge variant="outline" key={bank} className="ui-tag">{bank}</Badge>):<span className="muted text-xs">Aucune banque associée</span>}</div>
 {user&&<div className="client-actions flex-wrap gap-2"><Button variant="ghost" className="button-secondary" disabled={!can('clients.write')||deleting?.id===client.id} onClick={()=>setEditing(client)}><Pencil size={14}/> Modifier le client</Button><Button variant="ghost" className="icon-button danger-button" disabled={!can('clients.delete')||!can('accounts.delete')||deleting!==null} onClick={()=>setDeleting(client)} aria-label={`Supprimer ${client.name}`} title="Supprimer le client"><Trash2 size={16}/></Button></div>}</article></Card>)}</section>
